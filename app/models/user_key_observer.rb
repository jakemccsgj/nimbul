class UserKeyObserver < ActiveRecord::Observer
    ...
    def after_create(user_key)
        self.add_user_key_to_servers(user_key) if not user_key.public_key.blank? and user_key.user.enabled?
    end

    def before_destroy(user_key)
        self.delete_user_key_from_servers(user_key)
    end

    def before_update(user_key)
        @public_key_changed = user_key.public_key_changed?
        self.delete_user_key_from_servers(user_key) if @public_key_changed
    end

    def after_update(user_key)
        self.add_user_key_to_servers(user_key) if @public_key_changed and not user_key.public_key.blank? and user_key.user.enabled?
    end
