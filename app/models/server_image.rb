...
class ServerImage < BaseModel
    ...
    def self.refresh(server_image)
      Ec2Adapter.new(:account => server_image.provider_account).refresh_server_image(server_image)
    end
    ...
    def allocate!
        #begin
            if self.image_id
              Ec2Adapter.new(:account => self.provider_account).refresh_server_image(self)
            else
              Ec2Adapter.new(:account => self.provider_account).register_server_image(self)
            end
            self.save
        #rescue
        #    if self.location
    ...
    def release!
        begin
            if self.destroy
              Ec2Adapter.new(:account => self.provider_account).deregister_server_image(self)
            end
        rescue
            self.errors.add(:id, "#{$!}")
