class Parent::InstancesController < ApplicationController
  ...
  def index
    ...
    options = {
      ...
      :include => [ :zone, :server, :user, :security_groups, :provider_account, :instance_vm_type, :cpu_profile, :storage_type ],
    }
    @instances  = Instance.search_by_parent(parent, search, options)
