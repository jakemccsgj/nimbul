ActionController::Routing::Routes.draw do |map|
  ...
  map.resources :account_groups do |account_group|
    ...
    account_group.resources :stats,
      ...
      :only => [ :index, :total ]
    account_group.connect 'stats/:year',
      :year => /\d{4}/,
      :controller => 'parent/stats',
      :collection => { :total => :get },
      :only => [ :index, :total ]
    account_group.connect 'stats/:year/:month',
      :year => /\d{4}/, :month => /\d{2}/,
      :controller => 'parent/stats',
      :collection => { :total => :get },
      :only => [ :index, :total ]
  ...
  map.resources :provider_accounts,
    ...
    provider_account.resources :dns_leases,
      :controller => 'provider_account/dns_leases',
      :collection => { :release => :delete, :list => :any  },
      :except => [ :destroy, :edit, :new, :update, :create ]
    ...
    provider_account.connect 'stats/:year/:month',
      ...
      :collection => { :total => :get },
      :only => [ :index, :total ]
    provider_account.resources :server_image_categories,
      :collection => { :sort => :post }
    provider_account.resources :server_image_groups,
      :collection => { :sort => :post }
    ########
  ...
  map.resources :instances,
    ...
    instance.resource :servers,
      :controller => 'parent/servers',
      :only => [ :new, :create ]
    instance.resources :dns_leases,
      :controller => 'instance/dns_leases',
      :only => [ :release, :show ],
      :collection => { :release => :delete },
      :member => { :release => :delete }
    instance.resources :dns_hostnames,
      :controller => 'instance/dns_hostnames',
      :only => [ :acquire, :show ],
      :collection => { :acquire => :post },
      :member => { :acquire => :post }
    instance.resources :instance_resources,
      :controller => 'instance/instance_resources',
      :only => [ :create, :destroy, :update ],
      :collection => { :list => :any },
      :member => { :attach => :get, :detach => :get }
    instance.resources :addresses,
      :controller => 'instance/addresses',
      :only => [ :index, :list, :create, :destroy, :update ],
      :collection => { :list => :any }
    instance.resources :volumes,
      :controller => 'instance/volumes',
      :only => [ :index, :list, :create, :destroy, :update ],
      :collection => { :list => :any }
    instance.resources :audit_logs,
  ...
  map.resources :cloud_resources do |cloud_resource|
    cloud_resource.resources :clusters,
      :collection => { :auto_complete_for_cluster_id => :any },
      :controller => 'cloud_resource/clusters',
      :only => [ :new, :create, :destroy ]
    cloud_resource.resources :audit_logs,
      :controller => 'parent/audit_logs',
      :collection => { :list => :any },
      :only => [ :index ]
  end
  #map.cloud_resources_cluster_ac '/cloud_resource_cluster_ac',
  #    :controller => 'cloud_resource/clusters',
  ...
  # volumes
  map.resources :cloud_volumes,
    :collection => { :list => :any },
    :member => { :auto_complete_for_cluster_id => :any }

  # snapshots
  map.resources :cloud_snapshots,
    :collection => { :list => :any },
    :member => { :auto_complete_for_cluster_id => :any }
  ...
  map.resources :clusters,
    ...
    cluster.resources :stats,
      :controller => 'parent/stats',
      :collection => { :total => :get },
      :only => [ :index, :total ]
    cluster.connect 'stats/:year',
      :year => /\d{4}/,
      :controller => 'parent/stats',
      :collection => { :total => :get },
      :only => [ :index, :total ]
    cluster.connect 'stats/:year/:month',
      :year => /\d{4}/, :month => /\d{2}/,
      :controller => 'parent/stats',
      :collection => { :total => :get },
      :only => [ :index, :total ]
  ...
  map.resources :servers,
    ...
    server.resources :stats,
      ...
      :collection => { :total => :get },
      :only => [ :index, :total ]
    server.connect 'stats/:year',
      :year => /\d{4}/,
      :controller => 'parent/stats',
      :collection => { :total => :get },
      :only => [ :index, :total ]
    server.connect 'stats/:year/:month',
      :year => /\d{4}/, :month => /\d{2}/,
      :controller => 'parent/stats',
      :collection => { :total => :get },
      :only => [ :index, :total ]
